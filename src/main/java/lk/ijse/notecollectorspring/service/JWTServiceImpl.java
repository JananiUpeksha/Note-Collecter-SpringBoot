...
/* return genToken(new HashMap<>(), userDetails);*//*
   ...
   */
/* private String generateToken(Map<String, Object> genClaims, UserDetails userDetails) {
        genClaims.put("role", userDetails.getAuthorities());
        Date now = new Date();
        Date expiration = new Date(now.getTime() + 1000 * 600); // 10 minutes
        return Jwts.builder()
                .setClaims(genClaims)
                .setSubject(userDetails.getUsername())
                .setIssuedAt(now)
...
/*private String refreshToken(Map<String, Object> genClaimsRefresh, UserDetails userDetails) {
        genClaimsRefresh.put("role", userDetails.getAuthorities());
        Date now = new Date();
        Date refreshExpire = new Date(now.getTime() + 1000 * 600 * 600); // 10 hours
...
public class JWTServiceImpl implements JWTService {
   ...
   /* @Value("${spring.jwtKey}")
    ...
    private String generateToken(Map<String, Object> claims, UserDetails userDetails) {
        ...
        Date now = new Date();
        Date expiration = new Date(now.getTime() + 1000 * 3600); // 10 minutes
